...
## 2026-05-13 — Railway 云部署适配

### 改动文件

| 文件 | 改动 |
|------|------|
| `aion-chat/main.py` | PORT 从 `os.environ.get("PORT", 8080)` 读取（Railway 自动注入） |
| `aion-chat/requirements-railway.txt` | **新建**，去掉 `pywin32`（Windows 专属）和 `pyncm`（从 vendor 离线安装） |
| `railway.toml` | **新建**，Railway 部署配置（nixpacks builder，启动命令含 pyncm 离线安装） |
| `nixpacks.toml` | **新建**，系统依赖（PortAudio → sounddevice） |

### Railway 部署前提
- pyncm wheel 在 `vendor/pyncm-1.8.1-py3-none-any.whl`，部署时离线安装
- 数据持久化需在 Railway Dashboard 挂载 Volume 到 `/app/aion-chat/data/`

### 安全说明
- API Key 存在 SQLite 所在 volume 中，不会因重新部署丢失
- Railway 自带 HTTPS/边缘证书，无需额外配置
- 后续迁移到阿里云 VPS 时需额外配置防火墙 + nginx 反向代理
